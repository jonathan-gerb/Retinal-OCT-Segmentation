...
class BasicUNet(nn.Module):
    def __init__(
        ...
        self.recon_upcat_4 = UpCat(spatial_dims, fea[4], fea[3], fea[3], act, norm, bias, dropout, upsample)
        self.recon_upcat_3 = UpCat(spatial_dims, fea[3], fea[2], fea[2], act, norm, bias, dropout, upsample)
        self.recon_upcat_2 = UpCat(spatial_dims, fea[2], fea[1], fea[1], act, norm, bias, dropout, upsample)
        self.recon_upcat_1 = UpCat(spatial_dims, fea[1], fea[0], fea[5], act, norm, bias, dropout, upsample, halves=False)
        self.recon_final_conv = Conv["conv", spatial_dims](fea[5], out_channels, kernel_size=1)

        tasks = ["segmentation", "reconstruction", "classification", "curriculum"]
